...
registerProcessor(
  ...
  class extends AudioWorkletProcessor {
    constructor() {
      ...
      this.soundTouch.tempo = 1.2;
      this.port.onmessage = event => {
        if (event.data.type === "INIT") {
          this.channel0 = event.data.channel0;
          this.channel1 = event.data.channel1;
          this.soundTouch.tempo = event.data.tempo;
          this.source = {
            extract: (target, numFrames, position) => {
              /* this.emitter.emit("state", { t: position / this.context.sampleRate }); */
              const l = this.channel0;
              const r = this.channel1;
    ...
    process(inputs, outputs, parameters) {
      ...
      const framesExtracted = this.simpleFilter.extract(samples, bufferSize);
